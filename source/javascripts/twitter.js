...
window.addEvent('domready',function() {
  // getTwitterStatus(twitter_user);
	getTwitterSearch(twitter_search);
});
...
function parseTweetMeta(tweet_data){
  ...
  var user = tweet_data[3];
  var source = tweet_data[4];
  
  date = prettyDate(new Date().parse(date));
  return '<a href="http://twitter.com/' + user + '">' + user + '</a>: ' + tweet + '<span class="meta"><a href="http://twitter.com/'+user+'/statuses/'+tweet_id+'">' + date + '</a>' + (show_source ? ' from '+source : '') + '</span>';
  // return tweet + '<span class="meta">' + date + (show_source ? ' from '+source : '') + '</span>';
}
        ...
        the_tweets = Array();
  			tweets.each(function(tweet,i) {
  			  if((tweet.in_reply_to_status_id && show_replies) || !tweet.in_reply_to_status_id){
    			  ...
    			  the_tweets.push(tweet.text + key + tweet.created_at + key + tweet.id + key + tweet.from_user + key + tweet.source);
  				}
  			});
...
function getTwitterSearch(query){
  var container = $(twitter_container);
  if(!container) return;
  var tweet_cookie = 'edge_rails_tweets'
  container.set('html', 'Fetching tweets...');
  if(!Cookie.read(tweet_cookie)) {
  	var myTwitterGitter = new TwitterSearchGitter(query,{
  	  count: ((show_replies) ? tweet_count : 15 + tweet_count),
...
var TwitterSearchGitter = new Class({
	...
	retrieve: function() {
        ...
        // since_id: this.options.sinceID,
			...
			onComplete: function(data) {
				//linkify?
				if(this.options.link) {
					data.results.each(function(tweet) { tweet.text = this.linkify(tweet.text); },this);
				}
				//complete!
				this.fireEvent('complete',[data.results]);
			}.bind(this)
		}).request();
		return this;
	},
